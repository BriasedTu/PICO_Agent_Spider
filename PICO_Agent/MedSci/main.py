    ...
    def crawl(self, url):
        ...
        for i, url_sec in enumerate(url_secs):
            try:
                ...
                new_tab_ids = self.browser.tab_ids
                new_pdf_tab_id = [id for id in new_tab_ids if id != original_tab_id][0]
                
                # 切换到新标签页
                pdf_tab=self.browser.get_tab(new_pdf_tab_id)
                pdf_url = pdf_tab.url
                ...
                self.browser.close_tabs(ids=new_pdf_tab_id)
